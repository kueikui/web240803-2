    ...
    public partial class Login : System.Web.UI.Page
    {
        ...
        private void SendRandomPassword(string email, string code)
        {
            ...
            { 
                ...
                connection.Open();
                if (gap == 0) {
                    string query = "UPDATE CarerLogin SET cPassword=@cPassword WHERE cEmail=@cEmail";

                    MySqlCommand command = new MySqlCommand(query, connection);
                    command.Parameters.AddWithValue("@cEmail", forgetPass_Text.Text);
                    command.Parameters.AddWithValue("@cPassword", code);
                    command.ExecuteNonQuery();
                    connection.Close();
                }
                else if (gap == 1)
